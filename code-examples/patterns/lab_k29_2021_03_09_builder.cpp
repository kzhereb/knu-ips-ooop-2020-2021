...
TEST_CASE("building HTML using Builder pattern") {
	HtmlBuilder builder;
	SUBCASE("using regular API - each call as separate statement") {
	...
	SUBCASE("using fluent/chaining API - multiple calls chained together as a single statement") {
		builder.add_title("New Cars", 1)
				.add_title("Buy now!", 2)
				.add_text("Buy our new cars at considerable discount");
		builder.add_link("https://carsales.com/buy", "Buy a new car");
	}
